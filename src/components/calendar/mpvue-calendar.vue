    ...
    <table cellpadding="5">
      ...
      <div class="c-body">
        <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}" :class="{'gregorianStyle': !lunar}">
          <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled,'lunarStyle': lunar}"
            @click="select(k1,k2,$event)" class="day">
    ...
    methods: {
        ...
        for (i = 1; i <= lastDateOfMonth; i++) {
          ...
          if (this.range) {
            ...
            if (this.begin.length > 0) {
              let beginTime = Number(new Date(parseInt(this.begin[0]), parseInt(this.begin[1]) - 1, parseInt(this.begin[
                2])))
            ...
            if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(
                seletSplit[2]) == i) {
              temp[line].push(Object.assign({
                  day: i,
                  selected: true
      ...
      select(k1, k2, e) {
        ...
        if (this.range) {
          ...
          const filterDayIndex = this.multiDays.findIndex(v => this.year === v[0] && this.month === v[1] - 1 && this.days[
            k1][k2].day === v[2]);
          if (~filterDayIndex) {
